<?php
class ControllerProductCabledesign extends Controller {
	private $error = array();
		...
		if ($product_info) {
            ...
            if($this->config->get('module_live_options_status')){
                $data['module_live_options'] = array(
                    ...
                    'module_live_options_js' => $this->url->link('extension/module/live_options/js', 'product_id=' . (int)$product_id),
                );
			...
			$this->response->setOutput($this->load->view('product/cabledesign', $data));
		 } //else {
		// 	$url = '';
